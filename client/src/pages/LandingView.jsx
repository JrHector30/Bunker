...
const SaasButton = ({ variant = "default", className = "", children, ...props }) => {
  const baseStyles = "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-lg font-medium transition-all duration-200 focus:outline-none active:scale-95 text-sm h-11 px-5";
  const variants = {
    default: "bg-white text-black hover:bg-gray-100",
    ghost: "hover:bg-gray-900 text-gray-300 hover:text-white",
    gradient: "bg-gradient-to-b from-teal-400 to-teal-600 text-white hover:shadow-[0_0_25px_rgba(20,184,166,0.4)] font-semibold border border-teal-400/20"
  };
  return (
    ...
    <div className="landing-root min-h-screen bg-[#060609] text-white selection:bg-teal-500 selection:text-black overflow-x-hidden" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <header className="fixed top-0 w-full z-50 border-b border-gray-900/40 bg-[#060609]/70 backdrop-blur-md">
        <nav className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2 text-lg font-bold tracking-tight cursor-pointer" onClick={() => navigate('/')}>
        ...
        {mobileMenuOpen && (
          <div className="md:hidden bg-[#060609] border-t border-gray-900 px-6 py-6 flex flex-col gap-4">
      ...
      <section className="relative pt-36 pb-20 px-6 max-w-6xl mx-auto flex flex-col items-center text-center">
        <aside className="mb-6 inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-gray-800 bg-gray-900/20 backdrop-blur-sm text-xs text-gray-400">
        ...
        <div className="w-full max-w-5xl relative">
          ...
          <div className="relative z-10 border border-gray-800/60 rounded-2xl overflow-hidden shadow-[0_30px_70px_rgba(0,0,0,0.8)] bg-gray-950/60">
            <img
      ...
      <section id="features" className="py-24 px-6 max-w-6xl mx-auto border-t border-gray-900/40">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {features.map((f, i) => (
            <div key={i} className="p-6 rounded-xl border border-gray-900 bg-gray-950/30 backdrop-blur-xs transition-all duration-300 hover:border-gray-800 group">
              <div className="w-10 h-10 rounded-lg bg-teal-500/5 border border-teal-500/10 flex items-center justify-center text-teal-400 mb-4 group-hover:bg-teal-500/10 transition-colors">
                {f.icon}
              </div>
      ...
      <section id="workflow" className="py-24 px-6 bg-black/20 border-t border-gray-900/50">
        <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          ...
          <div className="flex flex-col gap-6">
            {steps.map((s, i) => (
              <div key={i} className="flex gap-4 p-5 rounded-xl border border-gray-900 bg-gray-950/20">
      ...
      <section id="pricing" className="py-24 px-6 max-w-6xl mx-auto border-t border-gray-900/40">
        <div className="text-center mb-12">
          ...
          <div className="inline-flex items-center p-1 rounded-lg bg-gray-950 border border-gray-900">
            <button className={`px-4 py-1.5 rounded-md text-xs font-medium transition-colors ${billingPeriod === 'monthly' ? 'bg-gray-900 text-white' : 'text-gray-400 hover:text-white'}`} onClick={() => setBillingPeriod('monthly')}>Mensual</button>
            <button className={`px-4 py-1.5 rounded-md text-xs font-medium transition-colors ${billingPeriod === 'yearly' ? 'bg-gray-900 text-white' : 'text-gray-400 hover:text-white'}`} onClick={() => setBillingPeriod('yearly')}>Anual (Ahorro)</button>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch max-w-5xl mx-auto">
          {pricing[billingPeriod].map((p, i) => (
            <div key={i} className={`p-8 rounded-2xl border flex flex-col justify-between relative transition-all duration-300 ${p.popular ? 'border-teal-500/50 bg-[#0a0f12] shadow-[0_0_30px_rgba(20,184,166,0.05)]' : 'border-gray-900 bg-gray-950/40'}`}>
              {p.popular && <span className="absolute top-4 right-4 bg-teal-500/10 border border-teal-500/20 text-teal-400 text-[10px] font-bold px-2.5 py-0.5 rounded-full uppercase tracking-wider">Más Popular</span>}
              <div>
                <h3 className="text-lg font-bold text-gray-200 mb-2">{p.name}</h3>
                ...
                <div className="flex items-baseline gap-1 mb-6">
                  ...
                  <span className="text-xs text-gray-500">/ {billingPeriod === 'monthly' ? 'mes' : 'año'}</span>
                </div>
                <div className="h-px bg-gray-900 mb-6" />
                <ul className="flex flex-col gap-3.5 mb-8">
                  {p.features.map((f, fi) => (
                    <li key={fi} className="flex items-start gap-2.5 text-xs text-gray-400 leading-tight">
              ...
              <SaasButton
                ...
                variant={p.popular ? "gradient" : "ghost"}
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {testimonials.map((t, i) => (
              <div key={i} className="p-6 rounded-xl border border-gray-900 bg-gray-950/20 flex flex-col justify-between">
                <div>
                  <div className="flex gap-0.5 mb-4 text-teal-400">
                    {Array.from({ length: t.stars }).map((_, si) => <Star key={si} fill="currentColor" size={14} />)}
                  </div>
                  <p className="text-sm text-gray-400 leading-relaxed italic mb-6">"{t.text}"</p>
                </div>
                <div className="flex items-center gap-3 border-t border-gray-900/60 pt-4">
      ...
      <section className="py-20 px-6 max-w-5xl mx-auto border-t border-gray-900/40 text-center">
        ...
        <div className="flex flex-wrap items-center justify-center gap-x-12 gap-y-6 opacity-30 select-none grayscale">
          {integrations.map((tech, i) => (
            <span key={i} className="text-base md:text-lg font-bold tracking-wider text-gray-400 lowercase" style={{ fontStyle: 'italic' }}>
              #{tech}
            </span>
          ))}
        </div>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex flex-col gap-3">
            {faqs.map((f, i) => {
              ...
              return (
                <div key={i} className="rounded-xl border border-gray-900 bg-[#050508] overflow-hidden transition-colors duration-200">
                  <button className="w-full p-5 flex items-center justify-between text-left font-medium text-sm md:text-base text-gray-200 hover:text-white" onClick={() => setActiveFaq(isOpen ? null : i)}>
                    <span>{f.q}</span>
                    <ChevronDown
                      className={`text-gray-500 transition-transform duration-200 ${isOpen ? 'rotate-180 text-teal-400' : ''}`}
                      size={16}
                    />
                  </button>
                  {isOpen && (
      ...
      <section className="relative py-24 px-6 border-t border-gray-900/40 overflow-hidden text-center">
        <div className="absolute left-1/2 w-[70%] h-[80%] pointer-events-none z-0 opacity-10 blur-[130px] bottom-[-20%] -translate-x-1/2 bg-teal-500 rounded-full" />
      ...
      <footer className="border-t border-gray-900 bg-black/60 py-12 px-6 text-xs text-gray-500">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6 text-center md:text-left">
          <div>
            <div className="flex items-center justify-center md:justify-start gap-2 text-sm font-bold text-white tracking-tight mb-2">
