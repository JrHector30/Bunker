...
export function CustomCharts({ arqueos, selectedArqueoId, onSelectArqueo }) {
  ...
  const maxIngreso = Math.max(...lastArqueos.map(a => a.totalBruto || 0), 100);
  const maxEgreso = Math.max(...lastArqueos.map(a => a.egresos || a.egreso || 0), 50);
  const maxOverall = Math.max(maxIngreso, maxEgreso) * 1.15; // 15% margin at top

  // Coordinate calculations
  const pointsIngreso = lastArqueos.map((arq, index) => {
    const x = padding + (index * (chartWidth - padding * 2)) / Math.max(1, lastArqueos.length - 1);
    const y = chartHeight - padding - ((arq.totalBruto || 0) * (chartHeight - padding * 2)) / maxOverall;
    return { x, y, val: arq.totalBruto || 0, id: arq.id, egreso: arq.egresos || arq.egreso || 0 };
  });

  const pointsEgreso = lastArqueos.map((arq, index) => {
    const x = padding + (index * (chartWidth - padding * 2)) / Math.max(1, lastArqueos.length - 1);
    const y = chartHeight - padding - (((arq.egresos || arq.egreso || 0) * (chartHeight - padding * 2)) / maxOverall);
    return { x, y, val: arq.egresos || arq.egreso || 0, id: arq.id };
  });

  // SVG Path generator helper
  const getAreaPath = (points, baseHeight) => {
  ...
  // Method Distribution Calculation (Derive from real db structure)
  const detailEfectivo = selectedArqueo.ingresos?.efectivo || 0;
  const detailDigital = (selectedArqueo.ingresos?.yape || 0) + (selectedArqueo.ingresos?.plin || 0);
  const detailTarjeta = (selectedArqueo.ingresos?.tarjeta || 0) + (selectedArqueo.ingresos?.izipay || 0) + (selectedArqueo.ingresos?.niubiz || 0);
  const detailManual = selectedArqueo.ingresos?.manual || 0;

  const methods = [
    { name: 'Efectivo', val: detailEfectivo, color: '#10b981', icon: DollarSign },
    { name: 'Digital (Yape/Plin)', val: detailDigital, color: '#3b82f6', icon: Smartphone },
    { name: 'Tarjeta (POS)', val: detailTarjeta, color: '#f59e0b', icon: CreditCard },
    { name: 'Manual/Otros', val: detailManual, color: '#64748b', icon: HelpCircle }
  ];
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
      ...
      <div id="chart-b" className="glass-panel p-4 flex flex-col justify-between transition-all">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-center">
          ...
          <div className="col-span-1 sm:col-span-6 flex flex-col justify-center space-y-3.5 font-sans">
            {methods.map((method) => {
