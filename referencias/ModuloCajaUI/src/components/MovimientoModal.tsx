...
import { X, Plus, DollarSign, ArrowUpRight, ArrowDownRight, CreditCard, Smartphone, HelpCircle, FileText } from 'lucide-react';
import { MovimientoCaja, Arqueo } from '../types';

interface MovimientoModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAddMovimiento: (mov: Omit<MovimientoCaja, 'id' | 'fecha' | 'arqueoId'>) => void;
  activeArqueo: Arqueo;
}

export function MovimientoModal({ isOpen, onClose, onAddMovimiento, activeArqueo }: MovimientoModalProps) {
  const [tipo, setTipo] = useState<'ingreso' | 'egreso'>('ingreso');
  const [monto, setMonto] = useState<string>('');
  const [metodoPago, setMetodoPago] = useState<'efectivo' | 'yape' | 'plin' | 'tarjeta'>('efectivo');
  const [comprobante, setComprobante] = useState<'boleta' | 'factura' | 'recibo'>('boleta');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const newErrors: { monto?: string; descripcion?: string } = {};
  ...
  const cajaDisponible = activeArqueo.totalCaja;
  const yapeDisponible = activeArqueo.ingresoDetalle.yape ?? 0;
  const plinDisponible = activeArqueo.ingresoDetalle.plin ?? 0;
        ...
        <div className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-lg transition-all sm:my-8 sm:w-full sm:max-w-md border border-slate-200">
          ...
          <div className="bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center text-slate-900">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-base font-bold text-slate-900">Registrar Movimiento</h3>
                <p className="text-[11px] text-slate-400">Arqueo en curso N° {activeArqueo.id}</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div>
              <label className="block text-xs font-semibold uppercase tracking-widest text-slate-400 mb-2">Tipo de Movimiento</label>
            ...
            {/* Monto Input */}
            <div>
              <label className="block text-xs font-semibold uppercase tracking-widest text-slate-400 mb-1.5">Monto (S/.)</label>
              <div className="relative rounded-lg">
                ...
                <input
                  ...
                  className={`block w-full rounded-lg border py-2.5 pl-9 pr-3 text-sm focus:outline-hidden transition-all ${errors.monto
                    ? 'border-rose-300 bg-rose-50/50 text-rose-900 focus:border-rose-500'
                    : 'border-slate-200 text-slate-900 focus:border-slate-800'
                    }`}
                />
              </div>
              {errors.monto && <p className="text-xs text-rose-600 mt-1">{errors.monto}</p>}
            </div>

            {/* Método de Pago */}
            <div>
              <label className="block text-xs font-semibold uppercase tracking-widest text-slate-400 mb-2">Método de Pago</label>
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <button
                    key={item.id}
                    type="button"
                    onClick={() => setMetodoPago(item.id as any)}
              ...
              {['efectivo', 'yape', 'plin'].includes(metodoPago) && (
                <div className="mt-2.5 p-2 rounded-lg bg-amber-50/70 border border-amber-200 text-amber-800 text-xs flex items-center gap-2 animate-fade-in">
            ...
            {/* Tipo de Comprobante */}
            <div>
              <label className="block text-xs font-semibold uppercase tracking-widest text-slate-400 mb-2">Tipo de Comprobante</label>
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <button
                    key={item.id}
                    type="button"
                    onClick={() => setComprobante(item.id as any)}
            ...
            {/* Descripción */}
            <div>
              <label className="block text-xs font-semibold uppercase tracking-widest text-slate-400 mb-1.5">Descripción / Concepto</label>
              <textarea
                rows={2}
                ...
                className={`block w-full rounded-lg border p-3 text-sm focus:outline-hidden transition-all resize-none ${errors.descripcion
                  ? 'border-rose-300 bg-rose-50/50 text-rose-900 focus:border-rose-500'
                  : 'border-slate-200 text-slate-900 focus:border-slate-800'
                  }`}
              />
              {errors.descripcion && <p className="text-xs text-rose-600 mt-1">{errors.descripcion}</p>}
            </div>
            ...
            <div className="pt-2 flex gap-3">
              <button
                type="button"
                onClick={onClose}
                className="flex-1 py-2.5 px-4 rounded-lg border border-slate-200 text-sm font-sans text-slate-700 bg-white hover:bg-slate-50 transition-colors cursor-pointer"
              >
                Cancelar
              </button>
              <button
                type="submit"
                className="flex-1 py-2.5 px-4 rounded-lg text-sm font-sans text-white bg-slate-900 hover:bg-slate-800 active:bg-slate-950 transition-colors cursor-pointer shadow-sm"
              >
                Confirmar Registro
              </button>
            </div>
