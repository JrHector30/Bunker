...
import { X, DollarSign, Smartphone, CreditCard, HelpCircle, FileText, Printer, ArrowUpRight, ArrowDownRight } from 'lucide-react';
import { Arqueo, MovimientoCaja } from '../types';
...
export function DetailModal({ isOpen, onClose, arqueo, movimientos }: DetailModalProps) {
  if (!isOpen || !arqueo) return null;

  // Filter movements for this specific arqueo
  const arqueoMovs = movimientos.filter((m) => m.arqueoId === arqueo.id);
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      ...
      <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
        <div className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-lg transition-all sm:my-8 sm:w-full sm:max-w-xl border border-slate-200">
          ...
          <div className="bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center text-slate-900">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400">Arqueo Sesión N° {arqueo.id} • {arqueo.estado}</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="p-6 space-y-5 max-h-[500px] overflow-y-auto">
            ...
            <div className="bg-slate-50 rounded-lg p-3 border border-slate-200 text-xs text-slate-600 grid grid-cols-2 gap-4">
              <div>
                ...
                <span className="font-mono font-medium text-slate-700">{arqueo.fechaInicio}</span>
              </div>
              <div>
                <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest">Fecha de Cierre</span>
                {arqueo.fechaCierre === 'EN CURSO' ? (
                  ...
                  <span className="font-mono font-medium text-slate-700">{arqueo.fechaCierre}</span>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-slate-50 border border-slate-200 p-3 rounded-lg text-center">
                ...
                <span className="text-sm font-bold font-mono text-slate-700">S/. {arqueo.inicio.toFixed(2)}</span>
              </div>
              <div className="bg-emerald-50/50 border border-emerald-200 p-3 rounded-lg text-center">
                ...
                <span className="text-sm font-bold font-mono text-emerald-700">S/. {arqueo.totalCaja.toFixed(2)}</span>
              </div>
              <div className="bg-slate-50 border border-slate-200 p-3 rounded-lg text-center">
                <span className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Total en Bruto</span>
                <span className="text-sm font-bold font-mono text-slate-700">S/. {arqueo.totalBruto.toFixed(2)}</span>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-3.5">
                {[
                  { name: 'Efectivo', val: arqueo.ingresoDetalle.efectivo, icon: DollarSign, color: 'text-emerald-500', bg: 'bg-emerald-50/20' },
                  { name: 'Digital (Yape/Plin)', val: arqueo.ingresoDetalle.digital, icon: Smartphone, color: 'text-blue-500', bg: 'bg-blue-50/20' },
                  { name: 'Tarjeta (Visa/MC)', val: arqueo.ingresoDetalle.tarjeta, icon: CreditCard, color: 'text-amber-500', bg: 'bg-amber-50/20' },
                  { name: 'Manual/Otros', val: arqueo.ingresoDetalle.manual, icon: HelpCircle, color: 'text-slate-500', bg: 'bg-slate-100/50' }
                ].map((item) => (
            ...
            <div className="grid grid-cols-2 gap-4 border-t border-slate-200 pt-3">
              <div>
                ...
                <div className="p-3 rounded-lg border border-amber-200 bg-amber-50/30 flex justify-between items-center">
                  ...
                  <span className="font-mono text-xs font-bold text-amber-700">S/. {arqueo.propinas.toFixed(2)}</span>
                </div>
              </div>
              <div>
                ...
                <div className="p-3 rounded-lg border border-rose-200 bg-rose-50/30 flex justify-between items-center">
                  ...
                  <span className="font-mono text-xs font-bold text-rose-700">S/. {arqueo.egreso.toFixed(2)}</span>
                </div>
              </div>
            </div>
                  ...
                  {arqueoMovs.map((mov) => (
                    <div key={mov.id} className="p-2.5 rounded-lg border border-slate-200 bg-slate-50 flex justify-between items-center text-xs">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <span className="font-semibold text-slate-700">{mov.descripcion}</span>
                          <span className="block text-[10px] text-slate-400 font-mono">{mov.fecha} • {mov.metodoPago}</span>
                        </div>
                      </div>
          ...
          <div className="bg-slate-50 px-6 py-4 flex gap-3 border-t border-slate-200 justify-end rounded-b-lg">
            <button
              onClick={() => {
                alert(`Imprimiendo reporte de arqueo N° ${arqueo.id}...`);
              }}
              className="flex items-center gap-1.5 py-2 px-3 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 text-xs font-semibold text-slate-700 transition-colors cursor-pointer"
            >
